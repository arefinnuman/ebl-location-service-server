...
const getSingleAdmin = (employeeId) => __awaiter(void 0, void 0, void 0, function* () {
    const isExist = yield admin_model_1.Admin.findOne({ employeeId });
    if (!isExist) {
        throw new apiError_1.default(http_status_1.default.NOT_FOUND, 'Admin not found !');
    }
    const result = yield admin_model_1.Admin.findOne({ employeeId }).populate('department');
    return result;
});
const updateAdmin = (employeeId, payload) => __awaiter(void 0, void 0, void 0, function* () {
    const isExist = yield admin_model_1.Admin.findOne({ employeeId });
    if (!isExist) {
        throw new apiError_1.default(http_status_1.default.NOT_FOUND, 'Admin not found !');
    }
    const { fullName } = payload, adminData = __rest(payload, ["fullName"]);
    ...
    if (fullName && Object.keys(fullName).length > 0) {
        Object.keys(fullName).forEach(key => {
            ...
            updatedAdminData[nameKey] =
                fullName[key];
        });
    }
    const result = yield admin_model_1.Admin.findOneAndUpdate({ employeeId }, updatedAdminData, {
        new: true,
    });
    return result;
});
const deleteAdmin = (employeeId) => __awaiter(void 0, void 0, void 0, function* () {
    const isExist = yield admin_model_1.Admin.findOne({ employeeId });
    if (!isExist) {
        throw new apiError_1.default(http_status_1.default.NOT_FOUND, 'Admin not found !');
    }
    ...
    try {
        ...
        const admin = yield admin_model_1.Admin.findOneAndDelete({ employeeId });
        if (!admin) {
